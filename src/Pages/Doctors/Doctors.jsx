...
const Doctors = () => {
  ...
  const { data: totalDoctors, isLoading } = useGetAllDoctorsQuery();
 ...
 return totalDoctors?.filter(c=>c.isActive==true)
  },[totalDoctors])
...
const applyFilter = () => {
  if (spaciality) {
    const matchedCategory = AllCategories?.find(c => c.name === spaciality);
    const filtered = allDoctors?.filter((doc) => doc.categoryId === matchedCategory?.id);
          ...
          <div className="flex flex-col gap-4 text-sm text-gray-600 animate-pulse">
            {isLoading
              ...
              : specialityList.map((spec) => (
                  <p
                    key={spec}
                    onClick={() => handleNavigation(spec.name)}
                    className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer ${
                      spaciality === spec.name
          ...
          <div className="w-full grid grid-cols-auto-cards gap-4 gap-y-6">
            {isLoading ? (
              Array.from({ length: 8 }).map((_, index) => (
                <div
                  key={index}
                  className="border border-gray-200 rounded-xl overflow-hidden animate-pulse bg-white"
                >
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row items-start gap-5 mt-5">
        ...
        <div className="flex flex-col gap-4 text-sm text-gray-600">
          {filteredDoctor?.map((spec) => (
                        <p 
                            key={spec.id}
                            onClick={() => handleNavigation(spec.name)} 
                            className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer ${
                                spaciality === spec.name ? "bg-indigo-50 text-black font-medium" : ""
                            }`}
                        >
                            {spec.name}
                        </p>
              ...
              {filterDoc.map((item, index) => (
                ...
                >
                  <img
                    src={item.image}
                    alt={item.name}
                    className="bg-blue-50 w-full object-cover max-h-60"
                  />
                  <div className="p-4">
                    <div className="flex items-center gap-2 text-sm text-center text-green-500">
                      <p className="w-2 h-2 bg-green-500 rounded-full"></p>
